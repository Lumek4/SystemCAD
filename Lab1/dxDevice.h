...
{
public:
	inline static DxDevice& Instance() { return *instance; }
	...
	mini::dx_ptr<ID3D11SamplerState> CreateSampler(const D3D11_SAMPLER_DESC& desc) const;
	...
	mini::dx_ptr<ID3D11RasterizerState> CreateRasterizerState(const RasterizerStateDescription& desc) const;
	...
	mini::dx_ptr <ID3D11Buffer> CreateIndexBuffer(const std::vector<T>& indices) const {
		auto desc = BufferDescription::IndexBufferDescription(
	...
	template<typename T>
	void SetBuffer(ID3D11Buffer* buf, T* val)
	{
		SetBuffer(buf, (void*)val, sizeof(T));
	}
	...
	mini::dx_ptr<ID3D11DomainShader> CreateDomainShader(std::vector<BYTE> gsCode) const;
